...
StatusLight::StatusLight( const Vector2i& position , std::wstring message , bool netUpdate ) :
        ...
        m_text( position , UIFont::getInstance().segoeUI14() , message , Colorf( 0 , 0 , 0 ) , Colorf( GetRValue(GetSysColor(COLOR_3DFACE)) , GetGValue(GetSysColor(COLOR_3DFACE)) , GetBValue(GetSysColor(COLOR_3DFACE)) ) , false ) {
...
void StatusLight::draw( HDC hdc ) {
    if ( hdc == NULL ) {
        ...
        for( double i = 0 ; i < 2 * M_PI ; i += M_PI / 12 ) {
            glVertex3f( getBoundingRect().left + radius + std::cos(i) * radius , getBoundingRect().top + radius + std::sin(i) * radius , 0 );
        }
        glEnd();
        ...
        radius = 10;
        for( double i = 0 ; i < 2 * M_PI ; i += M_PI / 12 ) {
            glVertex3f( getBoundingRect().left + (radius + 2) + std::cos(i) * radius , getBoundingRect().top + (radius + 2) + std::sin(i) * radius , 0 );
        }
        glEnd();
    }
    else {
