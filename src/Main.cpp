...
INT WINAPI WinMain(HINSTANCE Instance, HINSTANCE, LPSTR, INT) {
    ...
    gDrawables = new DisplaySettings("", 12, 12,
                                     gStreamWinPtr->getPosition().X + gStreamWinPtr->getSize().X + 10,
                                     12);
    ...
    while (!isExiting) {
        if (PeekMessage(&message, NULL, 0, 0, PM_REMOVE)) {
            ...
            if (std::chrono::system_clock::now() - connectTime  >
                std::chrono::milliseconds(2000) && !connectDlgOpen) {
            ...
            if (robotData.receive(dataPacket, receiveIP,
                                  receivePort) == sf::Socket::Done) {
                ...
                if (std::strcmp(header.c_str(), "display\r\n") == 0) {
                    ...
                    if (gAutonComboBox != NULL) {
                         ...
                         * new ones
                         */
                        while (SendMessage(gAutonComboBox, CB_DELETESTRING, 0,
                                           0) > 0) {
                        }
                        ...
                        for (unsigned int i = 0; i < autonNames.size(); i++) {
                            SendMessage(gAutonComboBox,
                                        CB_ADDSTRING,
                                        0,
                                        reinterpret_cast<LPARAM>(autonNames[i].
                                                                 c_str())
                                        );
                        }
                 ...
                 */
                else if (std::strcmp(header.c_str(),
                                     "autonConfirmed\r\n") == 0) {
...
LRESULT CALLBACK OnEvent(HWND handle, UINT message, WPARAM wParam,
    ...
    case WM_COMMAND: {
        ...
        case IDC_RELOAD_BUTTON: {
            ...
            if (gCmdSocketPtr != NULL) {
                gCmdSocketPtr->connect(robotIP, alfCmdPort, sf::milliseconds(
                                           500));
    ...
    case WM_PAINT: {
        ...
        HBITMAP gdiBmp =
            CreateCompatibleBitmap(gMainGLWin->m_dc, size.X, size.Y);
        ...
        glError = glGetError();
        if (glError != GL_NO_ERROR) {
            std::cerr << "Main.cpp OpenGL failure: " <<
        ...
        glError = glGetError();
        if (glError != GL_NO_ERROR) {
            std::cerr << "Drawables OpenGL failure: " <<
                gluErrorString(glError) << "\n";
        }
