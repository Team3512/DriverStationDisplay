...
void MjpegServer::start() {
    if (!m_isRunning) {
        ...
        m_isRunning = true;
        if (mjpeg_thread_create(&m_serverThread, &MjpegServer::serverFunc,
                                this) == -1) {
            m_isRunning = false;
        }
    }
...
void MjpegServer::stop() {
    if (m_isRunning) {
        ...
        for (std::list<mjpeg_socket_t>::iterator i = m_clientSockets.begin();
...
void* MjpegServer::serverFunc(void* obj) {
    ...
    while (objPtr->m_isRunning) {
        ...
        objPtr->m_clientSelector.socketsReady =
            objPtr->m_clientSelector.allSockets;

        // Wait until one of the sockets is ready for reading, or timeout is reached
        int count = select(objPtr->m_clientSelector.maxSocket + 1,
                           &objPtr->m_clientSelector.socketsReady,
                           NULL,
                           NULL,
                           &time);
        ...
        if (count > 0) {
            ...
            if (FD_ISSET(objPtr->m_listenSock,
                         &objPtr->m_clientSelector.socketsReady) != 0) {
                // Accept a new connection
                sockaddr_in acceptAddr;
                socklen_t length = sizeof(acceptAddr);
                mjpeg_socket_t newClient = accept(objPtr->m_listenSock,
                ...
                for (std::list<mjpeg_socket_t>::iterator i =
                         objPtr->m_clientSockets.begin();
                     i != objPtr->m_clientSockets.end();
                     i++) {
                        ...
                        if (recvSize > 0) {
                            ...
                            if (std::strncmp(tok, "GET", 3) == 0) {
                                ...
                                for (int length = 0;
                                    ...
                                    // Send a chunk of data
                                    sent = send(*i,
                                                ack.c_str() + length, sizeToSend - length,
                                                0);
