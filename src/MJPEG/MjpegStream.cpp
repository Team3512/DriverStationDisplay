...
MjpegStream::MjpegStream(const std::string& hostName,
    ...
    m_stopUpdate = false;
    if (mjpeg_thread_create(&m_updateThread, &MjpegStream::updateFunc,
                            this) == -1) {
        m_stopUpdate = true;
    }
}
...
void MjpegStream::setPosition(const Vector2i& position) {
    ...
    SetWindowPos(m_streamWin, NULL, position.X, position.Y,
                 getSize().X, getSize().Y, SWP_NOZORDER);

    // Set position of stream button below it
    SetWindowPos(m_toggleButton,
                 NULL,
                 position.X,
                 position.Y + 240 + 5,
                 100,
...
void MjpegStream::setSize(const Vector2i& size) {
    mjpeg_mutex_lock(&m_windowMutex);
    SetWindowPos(m_streamWin, NULL, getPosition().X,
                 getPosition().Y, size.X, size.Y, SWP_NOZORDER);
    mjpeg_mutex_unlock(&m_windowMutex);
...
void MjpegStream::startStream() {
    if (m_stopReceive == true) { // if stream is closed, reopen it
        ...
        m_streamInst = mjpeg_launchthread(
            const_cast<char*>(m_hostName.c_str()), m_port,
            const_cast<char*>(m_requestPath.c_str()), &m_callbacks);
...
void MjpegStream::saveCurrentImage(const std::string& fileName) {
    ...
    if (fileName.size() > 3) {
        ...
        if (toLower(extension) == "bmp") {
            // BMP format
            stbi_write_bmp(
                fileName.c_str(), m_imgWidth, m_imgHeight, 4, m_pxlBuf);
        }
            ...
            // PNG format
            stbi_write_png(
                fileName.c_str(), m_imgWidth, m_imgHeight, 4, m_pxlBuf, 0);
        }
        else {
...
void MjpegStream::readCallback(char* buf, int bufsize, void* optobj) {
    ...
    uint8_t* ptr = stbi_load_from_memory((unsigned char*) buf,
                                         bufsize,
                                         &width,
                                         &height,
                                         &channels,
    ...
    if (ptr && width && height) {
        ...
        if (std::chrono::system_clock::now() - streamPtr->m_displayTime >
            std::chrono::duration<double>(1.0 / streamPtr->m_frameRate)) {
            PostMessage(streamPtr->m_parentWin, WM_MJPEGSTREAM_NEWIMAGE, 0, 0);
            streamPtr->m_displayTime = std::chrono::system_clock::now();
        }
    }
    else {
        std::cout << "MjpegStream: image failed to load: " <<
            stbi_failure_reason() << "\n";
    }
}
...
void MjpegStream::recreateGraphics(const Vector2i& windowSize) {
    ...
    HBITMAP connectBmp = CreateCompatibleBitmap(streamWinDC,
                                                getSize().X, getSize().Y);
    HBITMAP disconnectBmp = CreateCompatibleBitmap(streamWinDC,
                                                   getSize().X, getSize().Y);
    HBITMAP waitBmp = CreateCompatibleBitmap(streamWinDC, getSize().X,
                                             getSize().Y);
    HBITMAP backgroundBmp = CreateCompatibleBitmap(streamWinDC,
                                                   getSize().X, getSize().Y);

    ReleaseDC(m_streamWin, streamWinDC);

    // Give each graphic a bitmap to use
    HBITMAP oldConnectBmp =
        static_cast<HBITMAP>(SelectObject(connectDC, connectBmp));
    HBITMAP oldDisconnectBmp =
        static_cast<HBITMAP>(SelectObject(disconnectDC, disconnectBmp));
    HBITMAP oldWaitBmp = static_cast<HBITMAP>(SelectObject(waitDC, waitBmp));
    HBITMAP oldBackgroundBmp =
        static_cast<HBITMAP>(SelectObject(backgroundDC, backgroundBmp));
    ...
    HRGN waitRegion = CreateRectRgn(windowSize.X / 3,
                                    windowSize.Y / 3,
                                    2 * windowSize.X / 3,
                                    2 * windowSize.Y / 3);
...
void* MjpegStream::updateFunc(void* obj) {
    ...
    while (!static_cast<MjpegStream*>(obj)->m_stopUpdate) {
        currentTime = std::chrono::system_clock::now() -
                      static_cast<MjpegStream*>(obj)->m_imageAge;
...
LRESULT CALLBACK MjpegStream::WindowProc(HWND handle,
                                         UINT message,
                                         WPARAM wParam,
                                         LPARAM lParam) {
    switch (message) {
    ...
    case WM_LBUTTONDOWN: {
        ...
        m_map[handle]->m_windowCallbacks->clickEvent(m_map[handle]->m_cx,
                                                     m_map[handle]->m_cy);
...
void MjpegStream::paint(PAINTSTRUCT* ps) {
    ...
    if (isStreaming()) {
        ...
        if (m_firstImage) {
            ...
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, getSize().X,
                            getSize().Y, GL_RGBA, GL_UNSIGNED_BYTE,
                            m_connectPxl);
    ...
    if (!m_stopReceive && std::strcmp(buttonText, "Start Stream") == 0) {
        ...
        SendMessage(m_toggleButton, WM_SETTEXT, 0,
                    reinterpret_cast<LPARAM>("Stop Stream"));
    }
    ...
    // If not running and button displays "Stop"
        ...
        // Change text displayed on button (LParam is button HWND)
        SendMessage(m_toggleButton, WM_SETTEXT, 0,
                    reinterpret_cast<LPARAM>("Start Stream"));
    }
}
