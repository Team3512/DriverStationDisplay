...
int mjpeg_sck_recv(int sockfd, void* buf, size_t len, int cancelfd) {
    ...
    while (nread < len) {
        ...
        error = select(mjpeg_max(sockfd,
                                 cancelfd) + 1, &readfds, NULL, &exceptfds,
                       NULL);
        if (error == -1) {
            return -1;
        }
 ...
 *  set appropriately. */
int mjpeg_sck_connect(char* host, int port, int cancelfd) {
    int sd;
    int error;
    int error_code;
    int error_code_len;
    ...
    FD_SET(sd, &exceptfds);
    FD_SET(cancelfd, &exceptfds);
    error = select(mjpeg_max(sd,
                             cancelfd) + 1, &readfds, &writefds, &exceptfds,
                   NULL);
    if (error == -1) {
        mjpeg_sck_close(sd);
        return -1;
...
struct keyvalue_t* mjpeg_process_header(char* header) {
    char* strtoksave;
...
void mjpeg_freelist(struct keyvalue_t* list) {
...
char* mjpeg_getvalue(struct keyvalue_t* list, char* key) {
    struct keyvalue_t* c;
    for (c = list; c != NULL; c = c->next) {
...
struct mjpeg_inst_t* mjpeg_launchthread(char* host,
                                        int port,
                                        char* reqpath,
