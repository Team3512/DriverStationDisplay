...
int mjpeg_thread_create(mjpeg_thread_t* thread, void*(*start_routine)(
                            void*), void* arg) {
#ifdef __WIN32
    thread->optarg = arg;
    thread->start_routine = start_routine;
    thread->thread =
        (HANDLE) (_beginthreadex(NULL, 0, win32_threadfunc, (void*) thread, 0,
                                 &thread->threadId));
    if ((int) thread->thread == -1) {
        return -1;
    }
    return 0;
#else
...
int mjpeg_thread_join(mjpeg_thread_t* thread, void** retval) {
#ifdef _WIN32
    int error;
