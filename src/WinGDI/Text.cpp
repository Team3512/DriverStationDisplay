...
Text::Text(const Vector2i& position,
           HFONT font,
           std::wstring text,
           Colorf fillColor,
           Colorf outlineColor,
...
void Text::draw(HDC hdc) {
    if (hdc != NULL) {
        ...
        COLORREF oldBackColor = SetBkColor(hdc,
                                           Drawable::getOutlineColor().win32());
        int oldMode = SetBkMode(hdc, TRANSPARENT);

        TextOutW(hdc, getPosition().X, getPosition().Y,
                 m_string.c_str(), static_cast<int>(m_string.length()));
